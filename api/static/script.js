...
form.addEventListener("submit", async (e) => {
  ...
  const clientPrice =
    parseInt(document.getElementById("clientPrice").value, 10) || null;
  const useAI = document.getElementById("useAI").value === "true";
  const numResults =
    parseInt(document.getElementById("numResults").value, 10) || 5;
  ...
  try {
    ...
    if (analogsData.length > 0) {
      ...
      const analogCard = document.getElementById("analogCard");
      if (analogCard) {
...
function render(data, clientPrice) {
  ...
  const minPrice = marketReport.market_range
    ? marketReport.market_range[0]
    : null;
  const maxPrice = marketReport.market_range
    ? marketReport.market_range[1]
    : null;
  ...
  if (minPrice && maxPrice) {
    document.getElementById(
      "rangeValue"
    ).textContent = `${formatPrice(minPrice)} – ${formatPrice(maxPrice)}`;
  } else {
...
function showAnalog(index) {
  ...
  if (!analogCard || !counterEl) return;
  ...
  currentAnalogIndex = Math.max(
    0,
    Math.min(index, analogsData.length - 1)
  );
  ...
  if (analog.pros && analog.pros.length > 0) {
    ...
    html +=
      '<div style="color: #4ade80; font-size: 11px; font-weight: 600; margin-bottom: 6px;">✓ ПЛЮСЫ</div>';
  ...
  if (analog.cons && analog.cons.length > 0) {
    ...
    html +=
      '<div style="color: #fb7185; font-size: 11px; font-weight: 600; margin-bottom: 6px;">✗ МИНУСЫ</div>';
    html += '<ul class="analog-list">';
  ...
  analogCard.innerHTML = html;
  updateAnalogCounter();

  if (prevBtn) prevBtn.disabled = currentAnalogIndex === 0;
  if (nextBtn) nextBtn.disabled =
    currentAnalogIndex === analogsData.length - 1;
}
...
function updateAnalogCounter() {
  ...
  const counter = document.getElementById("analogCounter");
...
function renderBestOriginal(data) {
  ...
  let html = `<div class="best-offer-title">`;

  if (bestOffer.title) {
    html += bestOffer.title;
  } else {
    html += "Лучшее объявление";
  }

  if (bestOffer.url) {
    html += ` <a href="${bestOffer.url}" target="_blank" rel="noopener noreferrer" class="best-offer-link">(перейти к объявлению)</a>`;
  }
...
function renderComparisonSlide() {
  const content = document.getElementById("bestComparisonContent");
  if (!content || !window.comparisonsArray || window.comparisonsArray.length === 0)
    return;

  const [analogName, comp] = window.comparisonsArray[
    window.currentComparisonIndex
  ];
  const data = window.lastAnalysisData;

  content.innerHTML = `<h3 style="margin: 0 0 12px 0; color: var(--text); font-weight: 600; padding-bottom: 12px; border-bottom: 1px solid var(--border);">🎪 Сравнение лучших объявлений</h3>`;
  ...
  html += `<div class="comparison-winner">🏆 ${
    comp.winner === "original" ? "Оригинал" : analogName
  }</div>`;
  html += `</div>`;

  html += `<div class="comparison-scores">`;
  html += `<div class="comparison-score">Оригинал: ${(comp.original_score || 0).toFixed(
    1
  )}/10</div>`;
  html += `<div class="comparison-score">Аналог: ${(comp.analog_score || 0).toFixed(
    1
  )}/10</div>`;
  html += `</div>`;
  ...
  if (comp.original_url) {
    ...
    html += `<a href="${comp.original_url}" target="_blank" rel="noopener noreferrer">${
      comp.original_title || comp.original_url
    }</a>`;
    html += `</div>`;
  }
  if (comp.analog_url) {
    ...
    html += `<a href="${comp.analog_url}" target="_blank" rel="noopener noreferrer">${
      comp.analog_title || comp.analog_url
    }</a>`;
    html += `</div>`;
  }
  html += `</div>`;
  ...
  if (comp.price_comparison) {
    ...
    const origPrice = pc.original_price
      ? pc.original_price.toLocaleString("ru-RU") + " ₽"
      : "—";
    const analogPrice = pc.analog_price
      ? pc.analog_price.toLocaleString("ru-RU") + " ₽"
      : "—";
    const diff = pc.difference_percent
      ? `${pc.difference_percent > 0 ? "+" : ""}${pc.difference_percent.toFixed(
          1
        )}%`
      : "";
    ...
    if (pc.monthly_payment_original) {
      html += ` (${pc.monthly_payment_original.toLocaleString(
        "ru-RU"
      )} ₽/мес)`;
    }
    html += `<br>Аналог: ${analogPrice}`;
    if (pc.monthly_payment_analog) {
      html += ` (${pc.monthly_payment_analog.toLocaleString(
        "ru-RU"
      )} ₽/мес)`;
    }
    if (diff) {
  ...
  const prevBtnComp = document.getElementById("prevComparisonBtn");
  const nextBtnComp = document.getElementById("nextComparisonBtn");
  if (prevBtnComp)
    prevBtnComp.disabled = window.currentComparisonIndex === 0;
  if (nextBtnComp)
    nextBtnComp.disabled =
      window.currentComparisonIndex === window.comparisonsArray.length - 1;
}

function updateComparisonCounter() {
  const counter = document.getElementById("comparisonCounter");
  if (counter) {
    const total = window.comparisonsArray
      ? window.comparisonsArray.length
      : 0;
...
// ===== КАРУСЕЛЬ СРАВНЕНИЙ (резервная версия, можно не вызывать) =====
...
function renderComparisonCarousel() {
  ...
  html += `<div class="comparison-scores">
    <div class="comparison-score">Оригинал: ${
      comp.original_score ? comp.original_score.toFixed(1) : 0
    }</div>
    <div class="comparison-score">Аналог: ${
      comp.analog_score ? comp.analog_score.toFixed(1) : 0
    }</div>
  </div>`;

  if (comp.price_comparison) {
    const pc = comp.price_comparison;
    html += `<div class="comparison-price"><strong>💰 Цена:</strong> ${
      pc.original_price ? pc.original_price.toLocaleString("ru-RU") : "—"
    } vs ${
      pc.analog_price ? pc.analog_price.toLocaleString("ru-RU") : "—"
    }</div>`;
  }
  ...
  const counter = document.getElementById("comparisonCounter");
  if (counter) {
    counter.textContent = `${currentComparisonIndex + 1}/${
...
function renderAllOffers(sources) {
  ...
  // РИСУЕМ все объявления (как было раньше)
  sources.forEach((offer, index) => {
    ...
    if (offer.url) {
      html += `<a href="${offer.url}" target="_blank" rel="noopener noreferrer" class="offer-title-link">${
        offer.title || "Объявление"
      }</a>`;
    } else {
      html += `<span class="offer-title">${
        offer.title || "Объявление"
      }</span>`;
    }
    html += `</div>`;
  ...
  // ВАЖНО: по умолчанию список скрыт
  section.classList.add("hidden");
  toggleBtn.textContent = `📋 Показать все объявления (${sources.length})`;

  // Обработчик кнопки: по клику показываем/скрываем ссылки
  toggleBtn.onclick = () => {
